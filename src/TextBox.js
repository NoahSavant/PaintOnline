...
TextBox.prototype.instantiate = function() {
    ...
    if (fb_rect.left <= tb_rect.right && tb_rect.left <= fb_rect.right && fb_rect.top <= tb_rect.bottom && tb_rect.top <= fb_rect.bottom) {
        $fb.css({
    ...
    function instantiate() {
        ...
        tb.$editor.focus();
        tb.$handles = $Handles(tb.$el, tb.$editor[0], {
            outset: 2
        });
        ...
        tb.$el.on("pointerdown", function(e) {
            if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement || e.target.classList.contains("handle")) {
                return;
            }
            e.preventDefault();

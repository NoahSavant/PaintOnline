...
function reset_file() {
    file_entry = null;
    file_name = "MS Paint : Microsoft Paint Online";
    update_title();
    saved = true;
}
...
function create_and_trigger_input(attrs, callback) {
    var $input = $(E("input")).attr(attrs).on("change", function() {
        callback(this);
        $input.remove();
    }).appendTo($app).hide().click();
    return $input;
}
...
function show_error_message(message, error) {
    ...
    if (error) {
        $(E("pre")).appendTo($w.$main).text(error.stack || error.toString()).css({
            background: "white",
            color: "#333",
...
function show_resource_load_error_message() {
    var $w = $FormWindow().title("Error").addClass("dialogue-window");
    $w.$main.html("<p>Failed to load image.</p>" + "<p>Make sure to use an image host that supports " + "<a href='https://en.wikipedia.org/wiki/Cross-origin_resource_sharing'>Cross-Origin Resource Sharing</a>" + ", such as <a href='https://imgur.com/'>Imgur</a>.");
    $w.$main.css({
        maxWidth: "500px"
    });
    $w.$Button("OK", function() {
...
function show_about_paint() {
    ...
    $about_paint_window.$content.append($about_paint_content.show()).css({
        padding: "15px"
    });
...
function paste(img) {
    if (img.width > canvas.width || img.height > canvas.height) {
        var $w = new $FormWindow().addClass("dialogue-window");
        $w.title("Paint");
        $w.$main.html("The image is bigger than the canvas.<br>" + "Would you like the canvas to be enlarged?<br>");
...
function render_history_as_gif() {
    ...
    try {
        ...
        gif.on("finished", function(blob) {
            ...
            $output.empty().append($(E("img")).attr({
                src: url,
                width: width,
                height: height,
            }));
...
function make_monochrome_pattern(lightness) {
    var dither_threshold_table = Array.from({
        length: 64
    }, function(undef, p) {
        var q = p ^ (p >> 3);
        return (((p & 4) >> 2) | ((q & 4) >> 1) | ((p & 2) << 1) | ((q & 2) << 2) | ((p & 1) << 4) | ((q & 1) << 5)) / 64;
    });
...
function set_as_wallpaper_centered(c) {
    ...
    if (window.chrome && chrome.wallpaper) {
        get_array_buffer_from_canvas(c).then(function(buffer) {
...
function sanity_check_blob(blob, okay_callback) {
    if (blob.size > 0) {
        ...
        $w.$main.html("<p>Tried to save file, but file was empty.</p>" + "<p>Try again, or if the problem persists, report here: " + "<a href='https://github.com/1j01/jspaint/issues/118'>Issue #118</a>");
        $w.$main.css({
            maxWidth: "500px"
        });
        $w.$Button("OK", function() {

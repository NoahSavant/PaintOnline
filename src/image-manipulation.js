...
function update_brush_for_drawing_lines(stroke_size) {
    if (aliasing && stroke_size > 1) {
        if (line_brush_canvas_rendered_shape !== "circle" || line_brush_canvas_rendered_color !== stroke_color || line_brush_canvas_rendered_size !== stroke_size) {
...
function draw_fill(ctx, x, y, fill_r, fill_g, fill_b, fill_a) {
    ...
    if (fill_r === start_r && fill_g === start_g && fill_b === start_b && fill_a === start_a) {
        return;
    }
    while (stack.length) {
    ...
    function matches_start_color(pixel_pos) {
        return (id.data[pixel_pos + 0] === start_r && id.data[pixel_pos + 1] === start_g && id.data[pixel_pos + 2] === start_b && id.data[pixel_pos + 3] === start_a);
    }
...
function stretch_and_skew(xscale, yscale, hsa, vsa) {
    apply_image_transformation(function(original_canvas, original_ctx, new_canvas, new_ctx) {
        ...
        new_ctx.save();
        new_ctx.transform(1, Math.tan(vsa), Math.tan(hsa), 1, -bb_x, -bb_y);
        new_ctx.drawImage(original_canvas, 0, 0, w, h);
        new_ctx.restore();
    });
}
...
(function() {
    ...
    function createShaderProgram() {
        var vertexSrc = ['attribute vec4 position;', 'void main() {', '    /* already in normalized coordinates, so just pass through */', '    gl_Position = position;', '}'].join('');
        var vertexShader = gl.createShader(gl.VERTEX_SHADER);
        gl.shaderSource(vertexShader, vertexSrc);
        gl.compileShader(vertexShader);
        if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
            console.log('Vertex shader failed to compile. Log: ' + gl.getShaderInfoLog(vertexShader));
        }
        var fragmentSrc = ['precision mediump float;', 'void main() {', '    gl_FragColor = vec4(0, 0, 0, 1);', '}'].join('');
        var fragmentShader = gl.createShader(gl.FRAGMENT_SHADER);
        gl.shaderSource(fragmentShader, fragmentSrc);
        gl.compileShader(fragmentShader);
        if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
            console.log('Fragment shader failed to compile. Log: ' + gl.getShaderInfoLog(fragmentShader));
        }
        ...
        if (stroke) {
            if (stroke_size > 1) {
                ...
                for (var i = 0; i < numPoints - (close_path ? 0 : 1); i++) {
                    ...
                    draw_line_without_pattern_support(op_ctx_2d, point_a.x - x, point_a.y - y, point_b.x - x, point_b.y - y, stroke_size);
                }

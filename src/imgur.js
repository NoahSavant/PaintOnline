...
function show_imgur_uploader(blob) {
    ...
    var $preview_image_area = $(E("div")).appendTo($imgur_window.$main);
    ...
    var $upload_button = $imgur_window.$Button("Upload", function() {
        ...
        var parseImgurResponseJSON = function(responseJSON) {
            try {
                ...
                $(E("pre")).appendTo($imgur_status).text(responseJSON).css({
                    background: "white",
                    color: "#333",
                ...
                $(E("pre")).appendTo($imgur_status).text(error.toString()).css({
                    background: "white",
                    color: "#333",
                    fontFamily: "monospace",
                    width: "500px",
        ...
        req.addEventListener("readystatechange", function() {
            if (req.readyState == 4 && req.status == 200) {
                ...
                $imgur_status.text("");
                var $imgur_url = $(E("a")).attr({
                    id: "imgur-url",
                    target: "_blank"
                });
                $imgur_url.text(url);
                $imgur_url.attr('href', url);
                $imgur_url_area.append("<label>URL: </label>").append($imgur_url);
